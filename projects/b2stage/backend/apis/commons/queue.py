...
def _log_any(instance, taskname, dictio, maris_params):
    ...
    if 'parameters' in maris_params and 'pids' in maris_params['parameters']:
        if len(maris_params['parameters']['pids']) > 3:
            tmp_maris_params = copy.copy(maris_params)               # must be shallow
            tmp_parameters = copy.copy(maris_params['parameters']) # must be shallow!
            few_pids = [
                maris_params['parameters']['pids'][0],
                maris_params['parameters']['pids'][1],
                '...'
            ]
            tmp_parameters['pids'] = few_pids
            tmp_maris_params['parameters'] = tmp_parameters
            maris_params = tmp_maris_params # only change pointer
